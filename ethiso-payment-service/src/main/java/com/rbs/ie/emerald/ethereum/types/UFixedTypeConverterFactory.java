...
public class UFixedTypeConverterFactory implements TypeConverterFactory<BigDecimal> {
	...
	public TypeConverter<BigDecimal> newTypeConverter(final TypeConverterManager typeConverterManager,
		...
		return new UFixedTypeConverter(sizes[0], sizes[1]);
	}
	...
	private static int[] getSizes(final String type) {
		...
		final String suffix = type.substring(UFIXED.length());
