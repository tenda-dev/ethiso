...
public class FixedTypeConverter implements TypeConverter<BigDecimal> {

	private static final BigDecimal TWO = new BigDecimal("2");
	private final BigDecimal twoPowerN;
	private final TypeConverter<BigInteger> intTypeConverter;
	private final int m;
	private final int n;

	public FixedTypeConverter(final int m, final int n) {
		if (m < 8 || n < 8 || m + n > 256 || m % 8 != 0 || n % 8 != 0) {
			throw new IllegalArgumentException(
					"Invalid sizes for " + FixedTypeConverter.class.getSimpleName() + " [m=" + m + ", n=" + n + "]");
		}

		this.twoPowerN = TWO.pow(n);
		this.intTypeConverter = new IntTypeConverter(m + n);
		this.m = m;
		this.n = n;
	}
	...
	public void serialiseType(final ByteBuffer byteBuffer, final BigDecimal value) {
		try {
			...
			intTypeConverter.serialiseType(byteBuffer, bigInteger);
	...
	public BigDecimal deserialise(final ByteBuffer byteBuffer) {
		try {
			final BigInteger bigInteger = intTypeConverter.deserialise(byteBuffer);
